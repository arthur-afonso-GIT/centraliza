...
export async function instalarApi(page: Page) {
  ...
  await page.route('**/api/**', async route => {
    ...
    if (url.pathname === '/api/compromissos/') {
      if (request.method() === 'POST') {
        const body = request.postDataJSON() as Record<string, unknown>;
        const created = { id: 3, ...body, participantes: [{ id: 2, nome: 'Inspetor de teste' }], demanda: body.demanda_id ?? null, cancelado_em: null };
    ...
    const appointment = url.pathname.match(/^\/api\/compromissos\/(\d+)\/$/);
    if (appointment && request.method() === 'PATCH') {
      const body = request.postDataJSON() as Record<string, unknown>;
      const index = appointments.findIndex(item => item.id === Number(appointment[1]));
      appointments[index] = { ...appointments[index], ...body, participantes: body.participante_ids ? [{ id: 2, nome: 'Inspetor de teste' }] : appointments[index].participantes };
      return route.fulfill({ json: appointments[index] });
    }
