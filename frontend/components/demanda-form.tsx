...
import { listarEquipes, type Equipe } from '../lib/equipe';

const vazio: DadosDemanda = { titulo: '', sei_numero: '', descricao: '', origem: '', prioridade: 'media', prazo: '', critica: false, responsavel_id: null, equipes_ids: [] };
...
export default function DemandaForm({ initial, onSaved }: { initial?: DemandaDetalhe; onSaved: (item: DemandaDetalhe) => void }) {
  ...
  const [equipes, setEquipes] = useState<Array<Pick<Equipe, 'id' | 'nome' | 'pode_administrar'>>>([]);
  ...
  function changeOpen(next: boolean) {
    setOpen(next);
    if (next) {
      setDados(initial ? { titulo: initial.titulo, sei_numero: initial.sei_numero, descricao: initial.descricao, origem: initial.origem, prioridade: initial.prioridade, prazo: initial.prazo, critica: initial.critica, responsavel_id: initial.responsavel?.id ?? null, equipes_ids: initial.equipes_ids ?? [] } : vazio);
  ...
  useEffect(() => {
    ...
    Promise.all([listarInspetores(controller.signal), listarEquipes(controller.signal)]).then(([people, teams]) => { setInspetores(people); setEquipes(teams.filter(team => team.pode_administrar)); }).catch(() => setError('Não foi possível carregar inspetores e equipes.')).finally(() => setLoadingPeople(false));
    return () => controller.abort();
  }, [open, initial]);
  const field = (key: keyof DadosDemanda, value: string | boolean | number | number[] | null) => setDados(current => ({ ...current, [key]: value } as DadosDemanda));
  const toggleEquipe = (id: number) => field('equipes_ids', dados.equipes_ids.includes(id) ? dados.equipes_ids.filter(item => item !== id) : [...dados.equipes_ids, id]);
