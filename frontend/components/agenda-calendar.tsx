...
import CancelarCompromisso from './cancelar-compromisso';
...
function EventCard({ item, manager, onChanged }: { item: Compromisso; manager: boolean; onChanged: () => void }) {
  return <article className={`calendar-event event-${item.tipo}`}><span>{item.tipo === 'reuniao' ? 'Reunião' : 'Atividade'}</span><strong>{item.titulo}</strong><time>{timeLabel(item.inicio)}–{timeLabel(item.fim)}</time>{manager && <div className="calendar-event-actions"><CompromissoForm initial={item} onSaved={onChanged} /><CancelarCompromisso item={item} onCancelled={onChanged} /></div>}</article>;
}

export default function AgendaCalendar({ user }: { user: User }) {
  ...
  function changed() { prepare(); setAttempt(value => value + 1); }
  ...
  return <section className="calendar" aria-labelledby="periodo-agenda">
    ...
    {agenda && agenda.results.length > 0 && view === 'dia' && <div className="day-view">{agenda.results.map(item => <EventCard item={item} manager={user.perfil === 'gestor'} onChanged={changed} key={item.id} />)}</div>}
    {agenda && agenda.results.length > 0 && view === 'semana' && <div className="week-view">{days.map(day => <section key={day} className={day === today ? 'is-today' : ''}><h3><span>{dateLabel(day, { weekday: 'short' })}</span>{dateLabel(day, { day: '2-digit' })}</h3><div>{agenda.results.filter(item => ocorreNoDia(item, day)).map(item => <EventCard item={item} manager={user.perfil === 'gestor'} onChanged={changed} key={item.id} />)}</div></section>)}</div>}
