...
export default function AvisosFeed() {
  ...
  if (!items?.length) return <section className="notices-state"><Bell size={36} /><h2>Nenhum aviso publicado</h2><p>Os comunicados da sua equipe aparecerão aqui.</p></section>;
  return <section className="notices-feed" aria-label="Avisos da equipe">{items.map(item => <Link className={`notice-card notice-${item.categoria}`} href={`/avisos/${item.id}`} key={item.id}><div className="notice-card-top"><span className={`notice-tag ${item.categoria}`}>{item.categoria === 'urgente' ? 'Urgente' : 'Informativo'}</span><time dateTime={item.publicado_em}>{data(item.publicado_em)}</time></div><h2>{item.titulo}</h2><p>{item.resumo}</p><div className="notice-meta"><span>Publicado por {item.autor}</span><span className="card-link">Ler aviso <ArrowRight size={17} /></span></div></Link>)}</section>;
}
