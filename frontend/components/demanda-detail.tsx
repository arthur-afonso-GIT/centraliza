...
import AvaliarDemanda from './avaliar-demanda';
...
export default function DemandaDetail({ id, user }: { id: number; user: User }) {
  ...
  async function refreshHistory() { try { setDemanda(await obterDemanda(id)); } catch { /* O anexo já foi salvo; a próxima abertura atualiza o histórico. */ } }
  ...
  return <main ref={contentRef} id="conteudo" className="content demand-detail" tabIndex={-1}>
    ...
    <div className="detail-grid"><div className="detail-column"><section className="detail-card" aria-labelledby="dados-demanda"><h2 id="dados-demanda">Informações</h2><p className="description">{demanda.descricao || 'Nenhuma descrição informada.'}</p><dl><div><dt>Número SEI</dt><dd>{demanda.sei_numero || 'Não informado'}</dd></div><div><dt>Origem</dt><dd>{demanda.origem || 'Não informada'}</dd></div><div><dt>Prazo</dt><dd><CalendarDays size={17} /> {data(demanda.prazo)}</dd></div><div><dt>Prioridade</dt><dd>{prioridadeLabel[demanda.prioridade]}</dd></div><div><dt>Responsável</dt><dd>{demanda.responsavel?.nome ?? 'Não atribuída'}</dd></div><div><dt>Equipes participantes</dt><dd>{demanda.equipes.length ? demanda.equipes.map(equipe => equipe.nome).join(', ') : 'Não vinculada'}</dd></div><div><dt>Criada em</dt><dd>{data(demanda.criada_em)}</dd></div></dl></section><AnexosDemanda demandaId={demanda.id} status={demanda.status} onHistoryChanged={refreshHistory} /></div>
      <section className="detail-card history" aria-labelledby="historico"><h2 id="historico">Histórico</h2><form className="comment-form" onSubmit={comment}><label htmlFor="comentario">Adicionar registro</label><textarea id="comentario" maxLength={2000} value={texto} onChange={event => setTexto(event.target.value)} placeholder="Escreva uma observação sobre a demanda" /><button className="secondary" disabled={busy || !texto.trim()}><MessageSquare size={17} /> Registrar comentário</button></form>{demanda.historico.length === 0 ? <p className="empty-history">Ainda não há eventos registrados.</p> : <ol className="timeline">{demanda.historico.map(event => { const actionLabel: Record<string, string> = { demanda_criada: 'Demanda criada', demanda_editada: 'Demanda editada', responsavel_alterado: 'Responsável alterado', anexo_adicionado: 'Anexo adicionado', anexo_removido: 'Anexo removido' }; return <li key={event.id}><span className="timeline-icon">{event.tipo === 'comentario' ? <MessageSquare /> : <Clock3 />}</span><div><strong>{event.tipo === 'comentario' ? 'Comentário registrado' : event.tipo === 'status_alterado' ? `${statusLabel[event.status_anterior]} → ${statusLabel[event.status_novo]}` : actionLabel[event.tipo]}</strong>{event.texto && <p>{event.texto}</p>}<small>{event.autor.nome} · {data(event.criado_em)}</small></div></li>; })}</ol>}</section></div>
  </main>;
}
