...
export default function DemandaDetail({ id, user }: { id: number; user: User }) {
  ...
  async function advance() {
    if (!demanda || (demanda.status !== 'pendente' && demanda.status !== 'em_andamento')) return;
    setBusy(true); setNotice(null);
    try {
      const next = demanda.status === 'pendente' ? 'em_andamento' : 'concluida';
      setDemanda(await alterarStatus(id, next));
      setNotice({ type: 'success', text: next === 'em_andamento' ? 'Demanda iniciada com sucesso.' : 'Demanda concluída com sucesso.' });
    } catch (reason) {
  ...
  const next = demanda.status === 'pendente' ? 'Iniciar demanda' : demanda.status === 'em_andamento' ? 'Concluir demanda' : null;
